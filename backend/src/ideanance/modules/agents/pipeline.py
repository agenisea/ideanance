...
from ideanance.core.security.sanitization import fence_user_content
from ideanance.modules.agents.base import (
    AgentRunMetadata,
    BaseIdeananceAgent,
)
from ideanance.modules.agents.deps import GovernanceDeps, RouterDeps
from ideanance.modules.agents.governance_filter import (
    GovernanceFilter,
    GovernanceFilterOutput,
)
from ideanance.modules.agents.query_router import (
    QueryRouter,
    RouteDecision,
    TaskType,
)
    ...
    async def execute(
        ...
        *,
        ...
        if self.handoff_manager:
            ...
            if handoff_response.status == "failed":
                return PipelineResult(
                    route=route,
